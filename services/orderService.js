...
export const createOrder = async (buyerId, payload) => {
    ...
    try {
        ...
        // Jika ADA SATU SAJA proses yang gagal (misal stok habis di tengah jalan), BATALKAN SEMUA perubahan!
        await t.rollback();

        // Lempar error ke Global Error Handler
...
export const shipOrder = async (orderId, sellerUserId, trackingNumber) => {
    ...
    // Otorisasi Kepemilikan: Pastikan yang mengubah adalah pemilik toko
    if (order.store.user_id !== sellerUserId) {
